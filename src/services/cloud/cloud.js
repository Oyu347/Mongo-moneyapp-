...
const FINANCIAL_COLLECTIONS=Object.freeze(['txns','budgets','goals','debts','invests','moneyAccounts','accountTransfers']);
...
function financialStateCoverage(state){const s=state&&typeof state==='object'?state:{};const present=FINANCIAL_COLLECTIONS.filter(k=>Object.prototype.hasOwnProperty.call(s,k)&&Array.isArray(s[k]));const missing=FINANCIAL_COLLECTIONS.filter(k=>!present.includes(k));return {present,missing,complete:missing.length===0};}
function partialReplacementRisk(input){const x=input||{},current=x.current&&typeof x.current==='object'?x.current:{},incoming=x.incoming&&typeof x.incoming==='object'?x.incoming:{};const currentCoverage=financialStateCoverage(current),incomingCoverage=financialStateCoverage(incoming);const lostCollections=currentCoverage.present.filter(k=>Array.isArray(current[k])&&current[k].length>0&&!incomingCoverage.present.includes(k));return {risky:lostCollections.length>0,lostCollections,currentCoverage,incomingCoverage};}
function safeReplacementDecision(input){const x=input||{};if(x.clearMarker||x.reason==='clear-tombstone')return {allow:true,reason:'clear-marker',lostCollections:[]};const risk=partialReplacementRisk({current:x.current,incoming:x.incoming});if(risk.risky)return {allow:false,reason:'partial-state',lostCollections:risk.lostCollections,risk};return {allow:true,reason:'safe-replacement',lostCollections:[],risk};}
global.MongoCloud=Object.freeze({MIRROR_REASONS,LEGACY_SOURCES,FINANCIAL_COLLECTIONS,stamp,chooseCloudOrLocal,candidateTime,sortNewestCandidates,makeCandidate,unwrapFinancialState,compactCandidates,selectLoadCandidate,requiresCanonicalMigration,chooseClearBarrier,queueItemAllowedAfterClear,filterQueueAfterClear,queueChangedByBarrier,mirrorRequired,shouldSkipFingerprint,tombstoneClearedAt,writeMetadata,financialStateCoverage,partialReplacementRisk,safeReplacementDecision});
})(window);
