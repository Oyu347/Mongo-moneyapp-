...
function normalizeBackEvent(event){const e=event||{};return Object.freeze({canGoBack:!!e.canGoBack});}
function fileCapabilities(env){return Object.freeze({filesystem:!!plugin('Filesystem',env),share:!!plugin('Share',env)});}
function capabilities(env){const w=env||global,c=capacitor(w),files=fileCapabilities(w);return Object.freeze({capacitor:!!c,native:isNative(w),platform:platform(w),touch:!!(w&&(('ontouchstart' in w)||(w.navigator&&Number(w.navigator.maxTouchPoints)>0))),safeAreaCss:true,appPlugin:!!plugin('App',w),keyboardPlugin:!!plugin('Keyboard',w),statusBarPlugin:!!plugin('StatusBar',w),filesystemPlugin:files.filesystem,sharePlugin:files.share});}
global.MongoMobile=Object.freeze({capacitor,isNative,platform,plugin,normalizeAppState,normalizeDeepLink,normalizeBackEvent,fileCapabilities,capabilities});
})(window);
