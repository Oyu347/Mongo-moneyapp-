...
function onVisible(handler,env){const w=env||global,d=w&&w.document;if(!(d&&typeof d.addEventListener==='function')||typeof handler!=='function')return false;d.addEventListener('visibilitychange',function(ev){if(!d.hidden)handler(ev);});return true;}
function onFocus(handler,env){const w=env||global;if(!(w&&typeof w.addEventListener==='function')||typeof handler!=='function')return false;w.addEventListener('focus',handler);return true;}
global.MongoWeb=Object.freeze({capabilities,canDownload,downloadText,readTextFile,reload,onPageShow,onVisible,onFocus});
})(window);
